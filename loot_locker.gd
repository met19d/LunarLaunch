...
var get_scores_http = HTTPRequest.new()				
...
func _authentication_request():
	...
	if player_id != null and player_id.length() > 1:
		print("player session exists, id="+player_id)
	...
	if(player_session_exists == true):
		data = { "game_key": game_API_key, "player_identifier":player_id, "game_version": "0.0.0.1", "development_mode": true }
...
func _get_scores():
	...
	var url = "https://api.lootlocker.io/game/leaderboards/1/list?count=5"
	var headers = ["Content-Type: application/json", "x-session-token:"+session_token]
	
	# Create a request node for getting the highscore
	get_scores_http = HTTPRequest.new()
	add_child(get_name_http)
	get_scores_http.request_completed.connect(_on_get_scores_completed)
	# Send request
	get_scores_http.request(url, headers, HTTPClient.METHOD_GET, "")
